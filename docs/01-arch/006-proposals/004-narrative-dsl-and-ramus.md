...
8. 编译产物、接口摘要和构建诊断使用显式的版本化合同；它们支持并行预编译，但不成为世界账本的一部分。
...
编译过程的诊断、缓存命中和性能统计属于可选的构建事件。它们可以按 CLI、GUI 或 CI 的需要过滤和丢弃。`WorldLedgerEntry` 则是世界事实的一部分，必须保留、版本化并参与回放。两者不得复用一个事件类型或存储出口。

并行构建、缓存键和依赖方向的详细约束见[编译制品、观测与边界治理](../../02-narrative-dsl/004-build-observability-and-boundaries.md)。
...
1. 完成 `WorldState`、`WorldPatch`、事件账本和存档合同。
2. 写出 capability catalog 的版本与迁移策略。
