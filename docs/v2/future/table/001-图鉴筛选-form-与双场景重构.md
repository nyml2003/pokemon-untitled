...
- 不引入运行时 SVG 渲染。筛选入口先使用 UTF-8 字符 `🔍`，不新增图标资源。
- 不改世界画面的 HUD，也不加返回按钮。
...
放大镜使用 UTF-8 字符 `🔍` 作为图标按钮。它同时表达“当前列表可筛选”和“已有筛选条件”。存在有效条件时，图标旁只显示属性图标、类别图标或短数值范围摘要，不显示长说明文字。
...
| `FilterSummary` | 紧凑态展示生效条件图标、结果数和放大镜。 | 只让放大镜和重置命令可操作，摘要项本身不可聚焦。 |
