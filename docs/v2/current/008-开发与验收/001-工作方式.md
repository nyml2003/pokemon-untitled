...
`ops lines` 使用 `tokei` 统计各 crate 的 `src/`、`examples/` 和 `tests/` Rust 代码。输出区分生产代码和测试代码，并列出代码行、注释行、空行和总行数。`ops coverage` 生成 workspace 覆盖率汇总，并写入 `target/coverage/html/index.html`。
