...
Cargo workspace 位于 `packages/ramus`，核心 crate 名为 `ramus-core`。
...
```powershell
cargo run --locked --manifest-path packages/ramus/Cargo.toml --example ramus -- "/battle/turn submit move=thunderbolt target=opponent:1"
```

不传命令时，example 使用上面的默认调用。它只使用内存 catalog、authorization 和 provider。

## 本地验证

```powershell
cargo fmt --all --manifest-path packages/ramus/Cargo.toml -- --check
cargo clippy --workspace --all-targets --locked --manifest-path packages/ramus/Cargo.toml -- -D warnings
cargo test --workspace --all-targets --locked --manifest-path packages/ramus/Cargo.toml
powershell -NoProfile -ExecutionPolicy Bypass -File packages/ramus/crates/ramus-core/scripts/Test-PureCoverage.ps1
```

覆盖脚本要求本机已有 nightly Rust 和 `cargo-llvm-cov`。它只统计 `boundary/` 之外的生产模块，并逐文件要求行覆盖和分支覆盖均为 100%。boundary 另由 capability matrix、provider failure、撤权和真实并发线性化测试验证。
