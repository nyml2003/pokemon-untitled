...
`game-view` 将世界、战斗、图鉴、命令控制台和页面模型投影为 `GameView` 或 `UiTree`。
它同时选择稳定的资源键和图层顺序。
...
使用 `project_pokedex`、`project_battle_ui`、`project_console_ui`、`project_foundation` 和 `project_page_model` 生成 `punctum-ui` 树。
`project_foundation` 提供旅程、背包和训练家卡片三页，产出的 `FoundationPageAction` 只描述用户意图；宿主负责将其交给 Ramus 路由。
`project_page_model` 只消费 `game-page-model::PageModel` 并产生 `PageIntent`；它不调用任何世界、地图或资源投影函数，因此页面 demo 可以在没有地图 renderer 的环境中解析。
