...
def build_parser() -> argparse.ArgumentParser:
    ...
    sync_parser.add_argument("--delete", action="store_true", dest="delete_removed")
...
def run(arguments: list[str] | None = None, source_root: Path | None = None) -> int:
    ...
    file_tree = LocalFileTree()
    sync_service = SyncService(file_tree, JsonMarkerStore())
    if args.command == "check":
        return _emit(sync_service.sync(config, delete_removed=False, dry_run=True), args.json_output)
    if args.command == "doctor":
        ...
        if not file_tree.is_directory(config.source_root.path):
            from tools.pokemon_ops.domain.errors import ErrorCode
        ...
        if not file_tree.is_directory(config.mirror_root.wsl_mount_path):
            from tools.pokemon_ops.domain.errors import ErrorCode

            return _emit(Result.fail(ErrorCode.MIRROR_MISSING, "mirror root does not exist", path=str(config.mirror_root.wsl_mount_path)), args.json_output)
    ...
    if args.command == "sync":
        return _emit(sync_service.sync(config, args.delete_removed, args.dry_run), args.json_output)
    ...
    return _emit(native.execute(config, operation, BuildProfile(args.profile)), args.json_output)
